...
class ControllerModuleKBMModRecentArticle extends Controller
{
	...
	public function index()
	{
		...
		if (isset($this->request->post['modules']))
		{
			...
			if ($kbm_recent_article = $this->model_setting_setting->getSetting('kbm_mod_recent_article', $this->data['selected_store_id']))
			{
				if (isset($kbm_recent_article['kbm_mod_recent_article_module']))
				{
					$modules = $kbm_recent_article['kbm_mod_recent_article_module'];
		...
		$this->template = 'module/kbm_mod_recent_article.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	private function save()
	{
		...
		$data = array(
			'kbm_mod_recent_article_module' => $this->request->post['modules']
		);

		$this->model_setting_setting->editSetting('kbm_mod_recent_article', $data, $this->request->post['store_id']);
		...
		{
			$this->redirect($this->helperLink('module/kbm_mod_recent_article', array('store_id' => $this->request->post['store_id'])));
		}
	}
