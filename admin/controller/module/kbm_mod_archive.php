...
class ControllerModuleKBMModarchive extends Controller
{
	...
	public function __construct($registry)
	{
		...
		ModelKulerCommon::loadTexts($this->language->load('module/kbm_mod_archive'));
	...
	public function index()
	{
		...
		$this->data['action'] = $this->helperLink('module/kbm_mod_archive');
		...
		if (isset($this->request->post['modules']))
		{
			...
			if ($kbm_archive = $this->model_setting_setting->getSetting('kbm_mod_archive', $this->data['selected_store_id']))
			{
				if (isset($kbm_archive['kbm_mod_archive_module']))
				{
					$modules = $kbm_archive['kbm_mod_archive_module'];
		...
		$this->data['moduleName'] = 'kbm_mod_archive';

		$this->document->setTitle(_t('heading_module_title'));

		$this->template = 'module/kbm_mod_archive.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	{
		$__ = $this->language->load('module/kbm_mod_archive');
	...
	private function getPathways() {
		...
		$breadcrumbs[] = array(
			...
			'href'      => $this->url->link('module/kbm_mod_archive', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);
	...
	{
		if (!$this->user->hasPermission('modify', 'module/kbm_mod_archive'))
		{
	...
	private function save()
	{
		...
		$data = array(
			'kbm_mod_archive_module' => $this->request->post['modules']
		);

		$this->model_setting_setting->editSetting('kbm_mod_archive', $data, $this->request->post['store_id']);
		...
		{
			$this->redirect($this->helperLink('module/kbm_mod_archive', array('store_id' => $this->request->post['store_id'])));
		}
	}
	...
	public function uninstall()
	{
		...
		{
			$this->model_setting_setting->deleteSetting('kbm_mod_archive', $store_id);
		}
	}
