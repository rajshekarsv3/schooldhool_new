...
class ControllerModuleKBMModArticleTag extends Controller
{
	...
	public function __construct($registry)
	{
		...
		ModelKulerCommon::loadTexts($this->language->load('module/kbm_mod_article_tag'));
	...
	public function index()
	{
		...
		$this->data['action'] = $this->helperLink('module/kbm_mod_article_tag');
		...
		if (isset($this->request->post['modules']))
		{
			...
			if ($kbm_article_tag = $this->model_setting_setting->getSetting('kbm_mod_article_tag', $this->data['selected_store_id']))
			{
				if (isset($kbm_article_tag['kbm_mod_article_tag_module']))
				{
					$modules = $kbm_article_tag['kbm_mod_article_tag_module'];
		...
		$this->data['moduleName'] = 'kbm_mod_article_tag';

		$this->document->setTitle(_t('heading_module_title'));

		$this->template = 'module/kbm_mod_article_tag.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	{
		$__ = $this->language->load('module/kbm_mod_article_tag');
	...
	private function getPathways() {
		...
		$breadcrumbs[] = array(
			...
			'href'      => $this->url->link('module/kbm_mod_article_tag', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);
	...
	{
		if (!$this->user->hasPermission('modify', 'module/kbm_mod_article_tag'))
		{
	...
	private function save()
	{
		...
		$this->kbm_model->deleteArticleTagsCache();

		$this->request->post['modules'] = isset($this->request->post['modules']) ? $this->request->post['modules'] : array();

		$data = array(
			'kbm_mod_article_tag_module' => $this->request->post['modules']
		);

		$this->model_setting_setting->editSetting('kbm_mod_article_tag', $data, $this->request->post['store_id']);
		...
		{
			$this->redirect($this->helperLink('module/kbm_mod_article_tag', array('store_id' => $this->request->post['store_id'])));
		}
	}
	...
	public function uninstall()
	{
		...
		{
			$this->model_setting_setting->deleteSetting('kbm_mod_article_tag', $store_id);
		}
	}
