...
class ControllerModuleKBMModCategory extends Controller
{
	...
	public function __construct($registry)
	{
		...
		ModelKulerCommon::loadTexts($this->language->load('module/kbm_mod_category'));
	...
	public function index()
	{
		...
		$this->data['action'] = $this->helperLink('module/kbm_mod_category');
		...
		if (isset($this->request->post['modules']))
		{
			...
			if ($kbm_category = $this->model_setting_setting->getSetting('kbm_mod_category', $this->data['selected_store_id']))
			{
				if (isset($kbm_category['kbm_mod_category_module']))
				{
					$modules = $kbm_category['kbm_mod_category_module'];
		...
		$this->data['moduleName'] = 'kbm_mod_category';

		$this->document->setTitle(_t('heading_module_title'));

		$this->template = 'module/kbm_mod_category.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	{
		$__ = $this->language->load('module/kbm_mod_category');
	...
	private function getPathways() {
		...
		$breadcrumbs[] = array(
			...
			'href'      => $this->url->link('module/kbm_mod_category', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);
	...
	{
		if (!$this->user->hasPermission('modify', 'module/kbm_mod_category'))
		{
	...
	private function save()
	{
		...
		$data = array(
			'kbm_mod_category_module' => $this->request->post['modules']
		);

		$this->model_setting_setting->editSetting('kbm_mod_category', $data, $this->request->post['store_id']);
		...
		{
			$this->redirect($this->helperLink('module/kbm_mod_category', array('store_id' => $this->request->post['store_id'])));
		}
	}
	...
	public function uninstall()
	{
		...
		{
			$this->model_setting_setting->deleteSetting('kbm_mod_category', $store_id);
		}
	}
