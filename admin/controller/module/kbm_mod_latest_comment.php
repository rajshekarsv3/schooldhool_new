...
class ControllerModuleKBMModLatestComment extends Controller
{
	...
	public function __construct($registry)
	{
		...
		ModelKulerCommon::loadTexts($this->language->load('module/kbm_mod_latest_comment'));
	...
	public function index()
	{
		...
		$this->data['action'] = $this->helperLink('module/kbm_mod_latest_comment');
		...
		if (isset($this->request->post['modules']))
		{
			...
			if ($kbm_latest_comment = $this->model_setting_setting->getSetting('kbm_mod_latest_comment', $this->data['selected_store_id']))
			{
				if (isset($kbm_latest_comment['kbm_mod_latest_comment_module']))
				{
					$modules = $kbm_latest_comment['kbm_mod_latest_comment_module'];
		...
		$this->data['moduleName'] = 'kbm_mod_latest_comment';

		$this->document->setTitle(_t('heading_module_title'));

		$this->template = 'module/kbm_mod_latest_comment.tpl';
		$this->children = array(
			'common/header',
			'common/footer'
		);
	...
	{
		$__ = $this->language->load('module/kbm_mod_latest_comment');
	...
	private function getPathways() {
		...
		$breadcrumbs[] = array(
			...
			'href'      => $this->url->link('module/kbm_mod_latest_comment', 'token=' . $this->session->data['token'], 'SSL'),
			'separator' => ' :: '
		);
	...
	{
		if (!$this->user->hasPermission('modify', 'module/kbm_mod_latest_comment'))
		{
		...
		if (isset($this->request->post['modules']))
		{
			...
			{
				if (empty($module['avatar_size']))
				{
					if (empty($this->errors['avatar_size']))
					{
						$this->errors['avatar_size'] = array();
					}

					$this->errors['avatar_size'][$module_index] = _t('error_avatar_size');
				}
			}
		}
	...
	private function prepareAlerts()
	{
		...
		$this->data['error_avatar_size'] = array();
	...
	private function save()
	{
		...
		$data = array(
			'kbm_mod_latest_comment_module' => $this->request->post['modules']
		);

		$this->model_setting_setting->editSetting('kbm_mod_latest_comment', $data, $this->request->post['store_id']);
		...
		{
			$this->redirect($this->helperLink('module/kbm_mod_latest_comment', array('store_id' => $this->request->post['store_id'])));
		}
	}
	...
	public function uninstall()
	{
		...
		{
			$this->model_setting_setting->deleteSetting('kbm_mod_latest_comment', $store_id);
		}
	}
