<?php echo $header; ?>
	...
	<script id="ModuleTemplate" type="text/x-handlebars-template">
		<div id="Module_{{module.row}}" class="vtabs-content">
			<table class="form">
				...
				<tr>
					...
					<td>
						<?php foreach ($languages as $language) { ?>
							<p>
								<input type="text" name="modules[{{module.row}}][title][<?php echo $language['language_id']; ?>]"<?php if ($language['language_id'] == $config_language_id) { ?> class="ModuleTitle" data-module-row="{{module.row}}" data-shortcode="#ModuleShortCode_{{module.row}}" <?php } ?>value="{{languageText module.title <?php echo $language['language_id']; ?>}}" size="50" />
				...
				<tr>
					<td><?php echo _t('entry_shortcode', 'Shortcode:'); ?></td>
					<td>
						<input type="text" id="ModuleShortCode_{{module.row}}" class="ModuleShortCode" name="modules[{{module.row}}][shortcode]" value="{{module.shortcode}}" readonly="readonly" size="40" />
					</td>
				</tr>
				<tr>
			...
			<table class="form">
				<tr>
					<td><?php echo _t('entry_product_featured_image'); ?></td>
					<td>
						<div class="kuler-switch-btn">
							<input type="hidden" name="modules[{{module.row}}][product_featured_image]" value="0" />
							<input type="checkbox" name="modules[{{module.row}}][product_featured_image]" value="1"{{#compare module.product_featured_image 1}} checked="checked"{{/compare}} />
							<span class="kuler-switch-btn-holder"></span>
						</div>
					</td>
				</tr>
				<tr>
					<td><?php echo _t('entry_product_description'); ?></td>
					<td>
						<div class="kuler-switch-btn">
							<input type="hidden" name="modules[{{module.row}}][product_description]" value="0" />
							<input type="checkbox" name="modules[{{module.row}}][product_description]" value="1"{{#compare module.product_description 1}} checked="checked"{{/compare}} />
							<span class="kuler-switch-btn-holder"></span>
						</div>
					</td>
				</tr>
				<tr>
					<td><?php echo _t('entry_specific_category'); ?></td>
					<td>
						<div class="scrollbox">
							...
							<?php foreach ($category_options as $category_id => $category_name) { ?>
								<div class="<?php echo $category_index % 2 ? 'odd' : 'even'; ?>">
									<input type="checkbox" name="modules[{{module.row}}][specific_categories][]" value="<?php echo $category_id; ?>"{{#inArray <?php echo $category_id; ?> module.specific_categories}} checked="checked"{{/inArray}} />
				...
				<tr>
					<td><?php echo _t('entry_article_limit'); ?></td>
					<td>
						<input type="text" name="modules[{{module.row}}][article_limit]" value="{{module.article_limit}}" size="5" />
					</td>
				</tr>
				<tr>
					<td><?php echo _t('entry_description_limit'); ?></td>
					<td>
						<input type="text" name="modules[{{module.row}}][description_limit]" value="{{module.description_limit}}" size="5" />
					</td>
				</tr>
				<tr>
					<td><?php echo _t('entry_featured_image_size'); ?></td>
					<td>
						<input type="text" name="modules[{{module.row}}][featured_image_width]" value="{{module.featured_image_width}}" size="5" /> x
						<input type="text" name="modules[{{module.row}}][featured_image_height]" value="{{module.featured_image_height}}" size="5"/>
						{{#isTrue 'ErrorsFeaturedImageSize' module.row}}
						<p class="error">{{prop 'ErrorsFeaturedImageSize' module.row}}</p>
						{{/isTrue}}
					</td>
				</tr>
			</table>
	...
	var g_token = '<?php echo $token; ?>',
		...
		ErrorsFeaturedImageSize = JSON.parse('<?php echo json_encode($error_featured_image_size); ?>');
